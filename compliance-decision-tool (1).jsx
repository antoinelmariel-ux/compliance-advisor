...
import { ChevronRight, ChevronLeft, AlertTriangle, CheckCircle, Settings, FileText, Users, Calendar, Info, Edit, Plus, Trash2, Eye } from 'lucide-react';
...
const analyzeAnswers = (answers, rules) => {
  ...
  return {
    ...
    risks: allRisks,
    complexity: allRisks.filter(r => r.level === 'Élevé').length > 2 ? 'Élevé' : 
                allRisks.filter(r => r.level === 'Élevé').length > 0 ? 'Moyen' : 'Faible'
  };
};
...
const QuestionnaireScreen = ({ questions, currentIndex, answers, onAnswer, onNext, onBack }) => {
  ...
  // Calculer le nombre de questions qui pourraient encore apparaître
  const totalPossibleQuestions = questions.length + 5; // estimation approximative
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {relevantTeams.map(team => {
                const teamPriority = analysis.risks.find(r => 
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center">
              ...
              Questions anticipées par équipe
            </h2>
            <div className="space-y-4">
              {Object.entries(analysis.questions).map(([teamId, teamQuestions]) => {
                const team = teams.find(t => t.id === teamId);
                return (
                  <div key={teamId} className="bg-white rounded-xl p-6 border border-gray-200">
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {analysis.risks.map((risk, idx) => (
                <div key={idx} className={`rounded-xl p-5 border-2 ${riskColors[risk.level]}`}>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-bold text-lg">{risk.description}</h3>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
            ...
            <ul className="space-y-3">
              <li className="flex items-start">
                ...
                <span className="text-gray-700">Contacter les équipes identifiées avec ce rapport</span>
              </li>
              <li className="flex items-start">
                <span className="text-green-600 font-bold mr-3">2.</span>
                <span className="text-gray-700">Préparer les réponses aux questions anticipées</span>
              </li>
              <li className="flex items-start">
                <span className="text-green-600 font-bold mr-3">3.</span>
                <span className="text-gray-700">Planifier les réunions de validation compliance</span>
              </li>
              <li className="flex items-start">
                <span className="text-green-600 font-bold mr-3">4.</span>
                <span className="text-gray-700">Mettre en place les actions de mitigation des risques</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="px-8 py-6 space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        ...
                        <div>
                          ...
                          {condition.question ? (
                            ...
                            >
                              <option value="">Sélectionner...</option>
                              {availableQuestions.find(q => q.id === condition.question)?.options.map((opt, i) => (
          ...
          {editedQuestion.conditions.length > 0 && (
            <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-200">
              <h4 className="font-semibold text-indigo-900 mb-2">📝 Aperçu de la logique</h4>
              <p className="text-sm text-indigo-800">
                La question "<strong>{editedQuestion.question}</strong>" s'affichera si :
              </p>
              <ul className="mt-2 space-y-1">
                {editedQuestion.conditions.map((cond, idx) => {
                  const refQuestion = availableQuestions.find(q => q.id === cond.question);
                  return (
                    <li key={idx} className="text-sm text-indigo-700 ml-4">
                      {idx > 0 && <strong>ET </strong>}
                      {refQuestion ? `"${refQuestion.question}"` : cond.question}{' '}
                      <strong>
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full my-8 max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-8 py-6 space-y-8">
          ...
          {editedRule.teams.length > 0 && (
            <div>
              <h3 className="text-xl font-bold text-gray-800 mb-4">❓ Questions que posera chaque équipe</h3>
              <div className="space-y-4">
                {editedRule.teams.map(teamId => {
                  const team = teams.find(t => t.id === teamId);
                  return (
                    <div key={teamId} className="bg-gray-50 rounded-lg p-5 border border-gray-200">
                      <div className="flex justify-between items-center mb-3">
                        <h4 className="font-bold text-gray-800">{team?.name}</h4>
                        <button
                          onClick={() => addQuestionForTeam(teamId)}
                          className="flex items-center px-3 py-1 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-all text-sm"
                        >
                          <Plus className="w-4 h-4 mr-1" />
                          Ajouter
                        </button>
                      ...
                      <div className="space-y-2">
                        {(editedRule.questions[teamId] || []).map((question, qIdx) => (
                          <div key={qIdx} className="flex items-center space-x-2">
                            <span className="text-gray-500 text-sm">{qIdx + 1}.</span>
                            <input
                              type="text"
                              value={question}
                              onChange={(e) => updateTeamQuestion(teamId, qIdx, e.target.value)}
                              className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"
                              placeholder="Question que l'équipe posera..."
                            />
                            <button
                              onClick={() => deleteTeamQuestion(teamId, qIdx)}
                        ...
                        {(!editedRule.questions[teamId] || editedRule.questions[teamId].length === 0) && (
                          <p className="text-sm text-gray-500 italic">Aucune question définie pour cette équipe</p>
                        )}
                      </div>
                    </div>
                  );
          ...
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">⚠️ Risques identifiés</h3>
              <button
                onClick={addRisk}
            ...
            {editedRule.risks.length === 0 ? (
              <div className="bg-gray-50 rounded-lg p-6 text-center text-gray-500">
                Aucun risque défini. Cliquez sur "Ajouter un risque" pour commencer.
              </div>
            ) : (
              <div className="space-y-3">
              ...
              <div className="space-y-4">
                {rules.map((rule) => (
                  <div key={rule.id} className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        <h3 className="text-lg font-bold text-gray-800 mb-2">{rule.name}</h3>
                        <div className="text-sm text-gray-600 mb-3">
                          <strong>Conditions:</strong> {rule.conditions.length} condition(s)
                        </div>
                        <div className="text-sm text-gray-600 mb-3">
                          <strong>Équipes déclenchées:</strong> {rule.teams.length > 0 ? rule.teams.join(', ') : 'Aucune'}
                        </div>
                        <div className="text-sm text-gray-600">
                          <strong>Risques:</strong> {rule.risks.length} risque(s) identifié(s)
                        </div>
                      </div>
                    ...
                    <div className="bg-white rounded p-4 border border-gray-200">
                      <p className="text-sm font-semibold text-gray-700 mb-2">Logique de la règle:</p>
                      {rule.conditions.length > 0 ? (
                        <div className="space-y-1">
                          {rule.conditions.map((cond, idx) => (
                            <div key={idx} className="text-sm text-gray-600">
                              {idx > 0 && <span className="text-indigo-600 font-semibold">ET </span>}
                              <span className="font-mono bg-gray-100 px-2 py-1 rounded">
                                {cond.question} {cond.operator === 'equals' ? '=' : cond.operator === 'not_equals' ? '≠' : 'contient'} "{cond.value}"
                              </span>
                            </div>
                          ))}
                        </div>
...
const ComplianceDecisionTool = () => {
  ...
  const [screen, setScreen] = useState('questionnaire'); // 'questionnaire' or 'synthesis'
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [answers, setAnswers] = useState({});
  const [analysis, setAnalysis] = useState(null);
  
  // Configuration (modifiable via back-office)
  ...
  const handleAnswer = (questionId, answer) => {
    ...
    // Nettoyer les réponses des questions qui ne seraient plus affichées avec cette nouvelle réponse
...
export default ComplianceDecisionTool;